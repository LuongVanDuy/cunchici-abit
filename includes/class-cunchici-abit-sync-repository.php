...
class Cunchici_Abit_Sync_Repository {
	public function upsert_candidate( array $mapped, array $raw ) {
		...
		$existing     = $wpdb->get_row( $wpdb->prepare( "SELECT id, payload_hash, sync_status FROM {$table} WHERE abit_product_id = %s", $abit_id ), ARRAY_A );
	...
	public function start_run( $run_id, $item = null ) {
		...
		$data = array(
			...
			'current_product_name' => $item ? $item['product_name'] : null,
		);
		$run = $this->get_run( $run_id );
		if ( $run && empty( $run['started_at'] ) ) {
			$data['started_at'] = current_time( 'mysql', true );
		}
		$wpdb->update( $table, $data, array( 'id' => absint( $run_id ) ) );
	}
